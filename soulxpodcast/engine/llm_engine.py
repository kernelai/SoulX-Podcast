...
class VLLMEngine:
    ...
    def __init__(self, model, **kwargs):
        ...
        if SUPPORT_VLLM:
            self.model = LLM(model=model, enforce_eager=True, dtype="bfloat16", max_model_len=8192, enable_prefix_caching=True, gpu_memory_utilization=config.gpu_memory_utilization)
        else:
